...
df['price_lakhs'] = df.apply(
    lambda x: x['price'] * 100 if x['price_unit'] == 'Cr' else x['price'], axis=1
)
...
print(f"Model ready — MAE: ₹{mae:.1f}L | R²: {r2:.3f}")
...
# ── Header ──
tk.Label(root, text="🏠 Mumbai House Price Predictor",
         font=FONT_TITLE, bg="#f5f5f5", fg="#1a1a1a").pack(pady=(20, 4))
tk.Label(root, text=f"Trained on {len(df):,} real listings  |  R² = {r2:.2f}  |  Avg error ±₹{mae:.0f}L",
         font=("Segoe UI", 8), bg="#f5f5f5", fg="#888").pack(pady=(0, 16))
...
def labeled_row(label_text):
    tk.Label(frame, text=label_text, font=FONT_LABEL,
             bg="#f5f5f5", anchor="w").pack(fill="x", pady=(10, 2))
...
for val in [1, 2, 3, 4, 5]:
    tk.Radiobutton(bhk_frame, text=str(val), variable=bhk_var, value=val,
                   font=FONT_LABEL, bg="#f5f5f5").pack(side="left", padx=6)
...
type_cb = ttk.Combobox(frame, textvariable=type_var, values=types,
                        state="readonly", font=FONT_LABEL, width=30)
type_cb.pack(anchor="w")

# ── Region with search ──
labeled_row("Region (type to search)")
region_var = tk.StringVar()
region_entry = tk.Entry(frame, textvariable=region_var, font=FONT_LABEL, width=34)
region_entry.pack(anchor="w")

region_listbox_frame = tk.Frame(frame)
region_listbox_frame.pack(anchor="w")
region_listbox = tk.Listbox(region_listbox_frame, height=4, width=34,
                             font=("Segoe UI", 9), selectmode="single")
region_scrollbar = tk.Scrollbar(region_listbox_frame, orient="vertical",
                                 command=region_listbox.yview)
...
ttk.Combobox(frame, textvariable=status_var, values=statuses,
             state="readonly", font=FONT_LABEL, width=30).pack(anchor="w")

# ── Age ──
labeled_row("Property Age")
age_var = tk.StringVar(value=ages[0])
ttk.Combobox(frame, textvariable=age_var, values=ages,
             state="readonly", font=FONT_LABEL, width=30).pack(anchor="w")
...
def predict():
    try:
        ...
        if price >= 100:
            display = f"₹ {price/100:.2f} Crore"
        else:
            display = f"₹ {price:.1f} Lakhs"
...
tk.Button(root, text="  Predict Price  ", font=("Segoe UI", 11, "bold"),
          bg="#1a6e2e", fg="white", relief="flat", cursor="hand2",
          padx=10, pady=8, command=predict).pack(pady=(14, 0))
